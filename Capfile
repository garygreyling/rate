...
namespace :deploy do
  ...
  task :start, :roles => :app do
    run "rm -rf /home/#{user}/public_html_addon/#{application}; ln -s #{current_path}/public /home/#{user}/public_html_addon/#{application}"
  end

  task :permissions, :roles => :app do
    run "chmod 755 #{current_path}/public; chmod 755 #{current_path}/public/.htaccess; chmod 755 #{current_path}/public/dispatch.fcgi; chmod -R 775 #{current_path}/log; chmod -R 775 #{current_path}/tmp"
  end
  after "shared:link_shared_public_directories", "deploy:permissions"
